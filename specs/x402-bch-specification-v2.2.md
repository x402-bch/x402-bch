...
| `txid`  | `string` | Required* | Transaction hash funding the reusable UTXO. Use `"*"` for "check my tab" mode. |
| `vout`  | `number` | Required* | Output index within `txid` that holds the funds. Use `null` for "check my tab" mode. |
| `amount`| `string` | Required* | Total satoshis originally sent in the funding UTXO (>= `value`). Use `null` for "check my tab" mode. |

\* `txid`, `vout`, and `amount` are required in standard mode but optional in "check my tab" mode (see section 5.2.3).

The Facilitator recomputes `JSON.stringify(authorization)` and checks it against the provided signature.

#### 5.2.3 Check My Tab Mode

Clients may use a special "check my tab" authorization format to request payment from any available UTXO associated with their address, without specifying a specific `txid` and `vout`. This enables automatic UTXO selection by the Facilitator.

**Authorization Format for Check My Tab Mode:**
...
```json
{
  ...
  "payload": {
    "signature": "IBvVHtN/5pArQTUI1WLJ1Y+lRXI+K3CbcGJH6YNCxZQoNjy0o4RD2bcHeU+9g3N+dlyNltc90QQTVDGc686wFaA=",
    "authorization": {
      "from": "bitcoincash:qz9s2mccqamzppfq708cyfde5ejgmsr9hy7r3unmkk",
      "to": "bitcoincash:qqlrzp23w08434twmvr4fxw672whkjy0py26r63g3d",
      "value": "1000",
...
- `txid` is set to `"*"` (string literal) to indicate automatic UTXO selection
- `vout` is set to `null` (not a number)
- `amount` is set to `null` (not a string)
- `value` remains required and specifies the amount to debit
- `from` and `to` remain required for signature verification and receiver validation

**Facilitator Behavior:**

When the Facilitator detects `txid === "*"`, it:

1. Queries its ledger for UTXOs associated with the `from` address that pay to the Server's `payTo` address
2. If no ledger entries exist, queries the blockchain for transactions from `from` to `payTo`
3. Selects the oldest available UTXO (FIFO) with sufficient balance to cover the requested `value`
4. If no valid UTXO is found, returns error code `no_utxo_found_for_address`
5. Once a UTXO is selected, normal debit tracking applies as if the Client had explicitly specified that UTXO

This mode is useful when Clients want to avoid tracking specific UTXOs and prefer the Facilitator to automatically manage UTXO selection and debiting.
...
- **Check My Tab Mode**: When using "check my tab" mode (see section 5.2.3), the authorization object includes `txid: "*"`, `vout: null`, and `amount: null`. The Facilitator must reconstruct the exact authorization object as signed by the Client, including these special values, when verifying the signature. The signature verification process is identical to standard mode; only the UTXO discovery process differs.
...
4. **UTXO Discovery**: 
   - **Standard Mode** (when `txid !== "*"`): Fetch the referenced UTXO via BCH APIs to confirm existence, ownership, and raw value.
   - **Check My Tab Mode** (when `txid === "*"`): 
     a. Query the Facilitator's ledger for UTXOs associated with the `from` address that pay to the Server's `payTo` address, ordered by `firstSeen` timestamp (oldest first, FIFO).
     b. If no ledger entries are found, query the blockchain for transactions from the `from` address to the Server's `payTo` address. For each transaction found, validate that the output pays to `payTo` and create a ledger entry.
     c. Select the oldest UTXO (by `firstSeen` timestamp or transaction confirmation time) that has sufficient remaining balance to cover the requested `value`.
     d. If no valid UTXO is found, return error code `no_utxo_found_for_address`.
     e. Once a UTXO is selected, proceed with standard verification using the selected `txid` and `vout`.
5. **Receiver Check**: Ensure the UTXO pays the Server's advertised `payTo` address.
6. **Balance Check**: Compare `utxoAmountSat` (or `remainingBalanceSat` from ledger) with requested debit; reject if insufficient.
...
**Note**: For efficient "check my tab" mode lookups, Facilitators SHOULD maintain a secondary index mapping payer addresses (`from`) to UTXO IDs (`utxoId`). This enables fast queries without scanning the entire ledger.
...
- **Address-Based Lookup (Check My Tab Mode)**: When using "check my tab" mode, the Facilitator queries the ledger by payer address (`from`) rather than by explicit `utxoId`. The ledger structure remains unchanged (still keyed by `utxoId = txid:vout`), but Facilitators should maintain a secondary index mapping `payerAddress` to `utxoId` for efficient queries. Once a UTXO is selected via address-based lookup, normal debit tracking applies as if the Client had explicitly specified that UTXO. The FIFO selection algorithm ensures predictable behavior by always using the oldest available UTXO first.
...
**Check My Tab Mode Request Example**

The following example shows a "check my tab" mode request where the Client uses `txid: "*"`, `vout: null`, and `amount: null` to request automatic UTXO selection:
...
The Facilitator will automatically select the oldest available UTXO associated with the `from` address that pays to the Server's `payTo` address. The response format is identical to standard mode.
...
- `no_utxo_found_for_address`: "Check my tab" mode was used but no valid UTXO exists for the payer address that pays to the Server's `payTo` address.
